...
import (
	...
	mock_bookly "github.com/pw-software-engineering/b-team/server/pkg/mocks/pkg/bookly"
...
func Test_api_handlePostOfferSimple(t *testing.T) {
	...
	type fields struct {
		logger       *zap.Logger
		offerService *mock_bookly.MockOfferService
	}
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			f := &fields{
				logger:       zap.NewNop(),
				offerService: mock_bookly.NewMockOfferService(ctrl),
			}
			if tt.prepare != nil {
				tt.prepare(t, f)
			}
